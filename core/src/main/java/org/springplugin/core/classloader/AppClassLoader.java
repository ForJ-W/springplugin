...
public class AppClassLoader extends URLClassLoader {
     ...
     * @author afěi
     */
    public AppClassLoader(String name, URL[] urls) {

        super(name, new URL[0], ClassUtils.currentClassLoader() instanceof AppClassLoader ? ClassUtils.currentClassLoader().getParent() : ClassUtils.currentClassLoader());
        for (URL url : urls) {
            try {
                addUrl(url);
            } catch (IOException e) {
    ...
    public static boolean isPluginClassLoader(@Nullable ClassLoader classLoader) {
        return classLoader instanceof SpringAppClassLoader;
    }
    ...
    public void close() throws IOException {
        final AppClassLoader remove = AppClassLoaderFactory.remove(getName());
